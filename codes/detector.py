        ...
        with self.detection_graph.as_default():
            ...
            if len(idx_vec) ==0:
                ...
                for idx in idx_vec:
                    ...
                    overlap = False
                    for item in tmp_car_boxes:
                        if helpers.IoM(item, box) > 0.75:
                            overlap = True


                    if (ratio < 0.8) and (box_h > 30) and (box_w > 30) and not overlap:
...
if __name__ == '__main__':
        ...
        det = CarDetector()
        os.chdir(cwd)
        TEST_IMAGE_PATHS= glob(os.path.join('part2/', '*.jpg'))
        ...
        for i, image_path in enumerate(TEST_IMAGE_PATHS[0:2]):
            ...
            start = time.time()
            b = det.get_localization(img_full_np, visual=True)
            end = time.time()
