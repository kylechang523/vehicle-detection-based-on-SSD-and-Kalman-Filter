...
def IoU(a, b):
    """
    Calculate ratio of Intersection over Union (IoU) of two boxes a, b
...
def IoM(a, b):
    """
    Calculate ratio of Intersection over Minimum (IoM) of two boxes a, b
    a[0], a[1], a[2], a[3] <-> left, up, right, bottom
    :param a:
    :param b:
    :return:
    ...
    return float(s_intsec) / min(s_a, s_b)
...
def draw_box_label(img, bbox_cv2, box_color=(0, 255, 255), show_label=True, id=None):
    ...
    if show_label:
        ...
        # Output the labels that show tracker id
        if id == None:
            ValueError('Please input tracker ID.')
        else:
            cv2.putText(img, 'id: ' + str(id), (left, top - 5), font, font_size, font_color, 1, cv2.LINE_AA)
